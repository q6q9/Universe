...
public:
	...
	//*/
	/*
	A* tree(const string& s, A* a, bool sort=false) {
		if (sort)
		{
			if (!a) {
				a = new A();
				...
				if (!x) {
					...
					a->right = tree(s, a->right, sort);
				}
				else {
					a->left = tree(s, a->left, sort);
				}
			}
...
public:
	...
	void print(A* a) {
		//cout << "\t\t\t 2459734762875683\t\t726\n";
		if ((!a) || (filt && ((a->s.size() % 2) == 1))) return; //Если ветки не существует - выходим. Выводить нечего
		tabs += 5; //Иначе увеличим счетчик рекурсивно вызванных процедур
		//Который будет считать нам отступы для вывода
		print(a->left); //Выведем ветку и ее подветки слева
		for (int i = 0; i < tabs; i++) cout << " "; //Потом отступы
